...
These three are enforced by the `trailing-whitespace` and `end-of-file-fixer` pre-commit hooks — run
`make precommit` (or install the hook once with `make precommit-install`) to apply them automatically.
