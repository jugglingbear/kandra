...
3. **Exclude non-check-in artifacts.** Skip scratch/generated files (anything under `temp/`, ad-hoc logs, one-off
   outputs) and anything the user said earlier not to commit. If a file's intent is ambiguous, flag it rather
