...
box. Point elsewhere with ``--url`` (or the ``BEAR_POKER_URL`` env var), e.g.
``--url http://192.168.1.1:8080`` for real hardware; clear a stale saved
identity with ``--reset``. The integration test
``tests/test_example_end_to_end.py`` exercises the same flow with in-process
fakes so CI needs no device.
"""
...
# Human label for the saved identity -- like naming a saved Wi-Fi network. You
# could save several devices under different names ("grizzly", "polar", ...).
SAVED_NAME = "grizzly"
...
async def first_run(url: str) -> None:
    ...
    # user input.
    _log("ENROLL", "POSTing /v1/auth/login and capturing the bearer token ...")
    ...
    # (`~/Library/Application Support/pneumatic_bear_poker_sdk/` on macOS,
...
async def subsequent_run(url: str) -> None:
    ...
    if not result.accepted:
        ...
        if result.classification.name == "TRANSPORT_FAILURE":
            # A saved identity pins the address captured at enrollment. If the
            # device has since moved (e.g. a new sim port), that address is dead
            # -- indistinguishable from "device offline", so re-enroll to refresh it.
